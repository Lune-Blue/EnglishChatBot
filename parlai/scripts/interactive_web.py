    ...
    <body>
        ...
        <script>
            function createChatRow(agent, text) {{
                ...
                icon.className = "fas fas fa-2x" + (agent === "You" ? " fa-user " : agent === "Model" ? " fa-robot" : agent === "Next Utterance" ? " fa-lightbulb" : agent === "Error Correction" ? " fa-exclamation" : " fa-robot");
            ...
            document.getElementById("interact").addEventListener("submit", function(event){{
                ...
                fetch('/interact', {{
                    ...
                    if (data.text.startsWith("Welcome to Convai Lab")){{
                        var final_result = data.text.split('|');
                        if (final_result.length == 1){{
                            parDiv.append(createChatRow("Model", data.text));
                            parDiv.scrollTo(0, parDiv.scrollHeight);
                        }}else{{
                            parDiv.append(createChatRow("Model", final_result[0]));
                            parDiv.append(createChatRow("Topic", final_result[1]));
                            parDiv.append(createChatRow("Bot's persona", final_result[2]));
                            parDiv.scrollTo(0, parDiv.scrollHeight);
                        }};                        
                    }}else{{
                        var final_result = data.text.split('|');
                        if (final_result.length == 1){{
                            ...
                            if(final_result[1] != "NO_CORRECTION"){{
                                console.log(final_result[1]);
                                console.log("NO_CORRECTION");
                                parDiv.append(createChatRow("Error Correction", final_result[1]));
                            }}
                            parDiv.append(createChatRow("Model", final_result[0]));
                            parDiv.append(createChatRow("Next Utterance", final_result[2]));
                            parDiv.scrollTo(0, parDiv.scrollHeight);
                        }};
                    }};
                }})
